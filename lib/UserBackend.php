...
class UserBackend extends ABackend implements ICheckPasswordBackend, IGetDisplayNameBackend, ICountUsersBackend {
    ...
    public function __construct($config, $userManager, $accountManager, $logger, $db, $appName) {
        $this->config = $config;
    ...
    private function storeUser($user) {
        if (!$this->userExists($user->username)) {
            ...
            $qb->update($this->table)
                ->set('displayname', $qb->createNamedParameter($user->first_name . ' ' . $user->last_name))
                ->where($qb->expr()->eq('uid', $qb->createNamedParameter($user->username)));
            $qb->execute();
            return true;
        }
        return false;
        ...
        if ($result['status'] == 200) {
            ...
            foreach($users as $user) {
                if ($this->storeUser($user)) {
                    $deleteUsers = array_values(array_diff($deleteUsers, [$user->username]));
                    $email = $user->email;
                    ...
                    if (!is_null($userObj)) {
                        ...
                        if ($currentEmail !== $email) {
                            ...
                            try {
                                $accountData = $this->accountManager->getUser($userObj);
                                $accountData[IAccountManager::PROPERTY_EMAIL]['scope'] = IAccountManager::VISIBILITY_PRIVATE;
                                $accountData[IAccountManager::PROPERTY_EMAIL]['verified'] = IAccountManager::VERIFIED;
                                $accountData[IAccountManager::PROPERTY_EMAIL]['value'] = $email;
                                $this->accountManager->updateUser($userObj, $accountData);
                            } catch (Exception $e) {
                                $this->logger->error($e, array('app' => $this->appName));
                            }
                        }
            ...
            foreach($deleteUsers as $user) {
                $this->logger->debug('Deleting user ' . $user, array('app' => $this->appName));
                $userObj = $this->userManager->get($user);
                $this->accountManager->deleteUser($userObj);
